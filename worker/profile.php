...
$query=mysqli_query($bd, "update workers set fullName='$fname',contactNo='$contactno',address='$address',State='$state',country='$country',pincode='$pincode' where userEmail='".$_SESSION['login']."'");
if($query)
{
...
<html lang="en">
  <head>
    ...
    <title>CMS | Worker Change Password</title>
 ...
 <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
                      ...
                      <?php }?>
 <?php $query=mysqli_query($bd, "select * from workers where userEmail='".$_SESSION['login']."'");
...
<label class="col-sm-2 col-sm-2 control-label">CategoryName</label>
 <div class="col-sm-4">
<input type="text" name="category" required="required" value="<?php echo htmlentities($row['categoryName']);?>" class="form-control" readonly>
</div>
</div>
<div class="form-group">
